...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE table_schema = 'public' 
      AND information_schema.columns.table_name = 'chat_participants'
...
DECLARE
  ...
  current_table TEXT;
BEGIN
  FOREACH current_table IN ARRAY tables_with_board_id
  LOOP
    ...
    IF EXISTS (
      ...
      -- 创建临时UUID列
      EXECUTE format('ALTER TABLE public.%I ADD COLUMN IF NOT EXISTS board_id_uuid UUID', current_table);
      ...
      EXCEPTION WHEN OTHERS THEN
        RAISE NOTICE 'Error converting TEXT to UUID in %: %', current_table, SQLERRM;
      END;
      
      -- 替换原始列
      BEGIN
        EXECUTE format('ALTER TABLE public.%I DROP COLUMN board_id', current_table);
        EXECUTE format('ALTER TABLE public.%I RENAME COLUMN board_id_uuid TO board_id', current_table);
      EXCEPTION WHEN OTHERS THEN
        RAISE NOTICE 'Error replacing board_id column in %: %', current_table, SQLERRM;
      END;
      
      RAISE NOTICE 'Converted board_id from TEXT to UUID in table %', current_table;
    END IF;
  END LOOP;
END
$$;
...
BEGIN
  IF NOT EXISTS (
    ...
    FROM information_schema.columns
    WHERE table_schema = 'public' 
      AND information_schema.columns.table_name = 'chat_rooms'

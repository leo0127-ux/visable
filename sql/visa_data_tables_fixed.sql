-- Create tables for storing visa data (with policy conflict handling)
...
BEGIN
    ...
    EXECUTE 'CREATE POLICY "Allow public to view H1B approvals" ON public.visa_h1b_approvals FOR SELECT USING (true)';
    EXECUTE 'CREATE POLICY "Allow public to view H1B denials" ON public.visa_h1b_denials FOR SELECT USING (true)';
    EXECUTE 'CREATE POLICY "Allow public to view H1B LCA" ON public.visa_h1b_lca FOR SELECT USING (true)';
    EXECUTE 'CREATE POLICY "Allow public to view PERM" ON public.visa_perm FOR SELECT USING (true)';
    EXECUTE 'CREATE POLICY "Allow public to view prevailing wages" ON public.visa_prevailing_wage FOR SELECT USING (true)';
    EXECUTE 'CREATE POLICY "Allow public to view metadata" ON public.visa_data_metadata FOR SELECT USING (true)';
    EXECUTE 'CREATE POLICY "Allow public to view summary" ON public.visa_data_summary FOR SELECT USING (true)';
EXCEPTION
    WHEN duplicate_object THEN
        RAISE NOTICE 'Policy already exists, continuing...';
END $$;
...
-- Insert initial summary record with explicit ID to avoid conflicts
INSERT INTO public.visa_data_summary (id, total_h1b_approvals, total_h1b_denials, approval_rate, avg_prevailing_wage, total_green_card_approvals)
VALUES ('00000000-0000-0000-0000-000000000001', 0, 0, 0, 0, 0)
ON CONFLICT (id) DO NOTHING;

-- Create an average wage calculation function for the dashboard

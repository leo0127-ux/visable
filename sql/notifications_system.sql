...
-- 安全地创建策略，避免"policy already exists"错误
DO $$
BEGIN
  IF NOT EXISTS (
...
BEGIN
  -- 获取帖子作者ID和标题
  SELECT author_id, title INTO post_author_id, post_title FROM public.posts WHERE id = NEW.post_id;
...
BEGIN
  ...
  LOOP
    -- 不要给作者自己发通知
    IF follower_record.user_id != NEW.author_id THEN
      INSERT INTO public.notifications (
        user_id, 
        type, 
        title, 
